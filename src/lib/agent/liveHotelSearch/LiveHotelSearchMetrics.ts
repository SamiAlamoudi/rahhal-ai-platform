/**
 * Sprint 109 — live hotel search metrics (structured, no secrets).
 */

export interface LiveHotelSearchMetricsSnapshot {
  searches: number
  successes: number
  failures: number
...
export class LiveHotelSearchMetrics {
  private searches = 0
  private successes = 0
  private failures = 0
  ...
  snapshot(): LiveHotelSearchMetricsSnapshot {
...
export function createLiveHotelSearchMetrics(): LiveHotelSearchMetrics {
  return new LiveHotelSearchMetrics()
}
