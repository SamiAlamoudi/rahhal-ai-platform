...
function softAck(facts: TravelFacts, seed: number, ar: boolean): string {
  ...
  if (ar) {
    if (d !== 'وجهتكم' && days && budget && couple) {
      return pick(seed, [
        `ميزانيتكم ${budget} ممتازة لرحلة ${days} أيام إلى ${d} مع زوجتكم.`,
        `${d} لأسبوع مع شريكتكم وبميزانية ${budget} — إطار واضح وجميل نقدر نبني عليه.`,
      ])
    }
    if (d !== 'وجهتكم' && days) {
      return pick(seed, [
...
function moroccoStyleAdvice(facts: TravelFacts, seed: number): LocalConversationResult | null {
  ...
  const opening = budget
    ? `ميزانيتكم ${budget} ممتازة لرحلة ${days} أيام إلى المغرب.`
    : `رحلة ${days} أيام إلى المغرب إطار ممتاز لنبدأ منه.`

  const body = pick(seed, [
    [
      opening,
      'إذا كنتم تبحثون عن الاسترخاء فالبحر في أكادير يمنحكم هدوءًا أسهل.',
      'أما إذا فضّلتم الثقافة والأسواق التقليدية فمراكش خيار رائع.',
      'بعد أن تختاروا المدينة سأجهّز لكم أفضل الرحلات والفنادق المناسبة بهدوء ودقة.',
    ].join('\n\n'),
    [
      opening,
      'مراكش تناسبكم إن أحببتم الأجواء الحية والأسواق.',
      'وأكادير أنسب إن كان الهدف راحة وشاطئ بإيقاع أهدأ.',
      'أخبروني أيّ المدينتين أقرب لكم حتى أرتّب الطيران والإقامة على ميزانيتكم.',
    ].join('\n\n'),
  ])
  ...
  const spoken = optimizeLocalSpoken(
    [
      opening,
      'إذا أردتم الاسترخاء فأرشح أكادير، وللثقافة والأسواق مراكش خيار رائع.',
      'بعد اختيار المدينة أجهّز لكم أفضل الرحلات والفنادق.',
    ].join(' '),
    'ar',
  )
