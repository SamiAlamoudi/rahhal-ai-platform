...
describe('CheckoutOrchestrator', () => {
  ...
  it('executes full payment flow: authorize then capture then confirm', async () => {
    ...
    const result = await orchestrator.executePayment(session.order.id, session.lockToken!)
    expect(result.success).toBe(true)
  ...
  it('prevents duplicate payment with lock', async () => {
    ...
    await orchestrator.createPaymentSession(session.order.id, null, null, 'https://example.com/return')
    const result1 = await orchestrator.executePayment(session.order.id, session.lockToken!)
    expect(result1.success).toBe(true)
    // Second attempt with same lock token should fail — lock released after first payment
    const result2 = await orchestrator.executePayment(session.order.id, session.lockToken!)
  ...
  it('fails executePayment without valid lock token', async () => {
    ...
    await orchestrator.createPaymentSession(session.order.id, null, null, 'https://example.com/return')
    const result = await orchestrator.executePayment(session.order.id, 'invalid-token')
    expect(result.success).toBe(false)
  ...
  it('retries failed payment after releasing lock', async () => {
    ...
    const failedResult = await orchestrator.executePayment(session.order.id, 'wrong-token')
  ...
  it('cannot retry a paid order', async () => {
    ...
    await orchestrator.createPaymentSession(session.order.id, null, null, 'https://example.com/return')
    await orchestrator.executePayment(session.order.id, session.lockToken!)
  ...
  it('order is created before payment authorization', async () => {
    ...
    const execResult = await orchestrator.executePayment(session.order.id, session.lockToken!)
