...
export function ConversationController(
  ...
  async function runTurn(input: ConversationTurnInput): Promise<ConversationTurnResult> {
    ...
    // Sprint 43 — multi-tool orchestration skips clarifying when destination is already known.
    const orchestratorWillHandle =
      aiOrchestrator.isEnabled()
      && aiOrchestrator.shouldHandle({
        userText: input.userText,
        commandHint: commandKind,
      })
    const skipClarifyingForOrchestrator =
      orchestratorWillHandle
      && (
        Boolean(state.context.destination)
        || /travel to|visit|trip to|morocco|japan|dubai|paris|london|أريد السفر|رحلة إلى/i.test(
          input.userText,
        )
        || commandKind === 'flight_cancelled'
        || commandKind === 'make_cheaper'
        || /cheapest|lost (my )?passport|أرخص|فقدت جواز/i.test(input.userText)
      )

    if (
      followUps.shouldAskBeforePlanning(state)
      && !skipClarifyingForCommand
      && !skipClarifyingForOrchestrator
    ) {
    ...
    if (orchestratorWillHandle) {
      ...
      try {
        const orch = await aiOrchestrator.run({
          conversationId: input.conversationId,
          userText: input.userText,
          locale,
          userId: input.userId,
        ...
        state = {
          ...
          phase: 'presenting',
          pendingFollowUpField: null,
          lastPlanResult: orch.planResult ?? state.lastPlanResult,
          context: {
            ...state.context,
            destination:
              state.context.destination
              || (typeof orch.uiMeta.memory === 'object'
                && orch.uiMeta.memory
                && 'destination' in (orch.uiMeta.memory as object)
                ? String((orch.uiMeta.memory as { destination?: string | null }).destination ?? '')
                  || state.context.destination
                : state.context.destination),
          },
        }
        const structured = orch.structured
        const renderedText = orch.text
        const assistantMessage = createConversationMessage({
          conversationId: input.conversationId,
          role: 'assistant',
          content: renderedText,
          ...
          meta: {
            ...
            conversationUi: true,
            observability: {
              selectedTools: orch.observability.selectedTools,
              executionTimeMs: orch.observability.executionTimeMs,
              intent: orch.observability.plannerDecisions.intent,
              fallbackReasons: orch.observability.fallbackReasons,
              errors: orch.observability.errors,
            },
          },
        })
        session = appendMessage(updateSessionState(session, state), assistantMessage)
        ...
        return {
          ...
          structured,
          renderedText,
          planResult: orch.planResult,
          commandKind,
          durationMs: Date.now() - started,
        }
      } catch (error) {
        events.emit(createConversationEvent('error', input.conversationId, {
          message: error instanceof Error ? error.message : 'orchestrator_failed',
          aiOrchestrator: true,
        }))
        // Fall through to existing single-tool / planner paths.
      }
    }
