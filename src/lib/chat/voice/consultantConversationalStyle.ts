...
export function buildConsultantConversationalInstructions(input: {
  ...
  return [
    ...
    '  1) destination  2) dates or flexibility  3) traveler count (soft-default solo)',
    '- Origin/departure: NEVER ask "من أي مدينة؟" when a default airport is already assumed (geolocation or memory).',
    '- If departure is assumed, you may briefly confirm: "سأعتبر الرياض نقطة الانطلاق." and offer تغيير المطار — do not interrogate.',
    '- Only ask origin when NOTHING is known (no geo, no memory, no stated city).',
    ...
    '- As soon as destination + dates are known: STOP interviewing and SEARCH / show bookable options.',
    ...
    '- Good Arabic: "والتواريخ تقريبًا؟" / "سأعتبر الرياض نقطة الانطلاق." — never mix English into Arabic turns.',
    '- Bad: long talk about Bangkok, Sukhumvit, Koh Samui, Chaweng, areas, vibes, "book early", "trusted companies".',
    '- Never expose raw reasoning, chain-of-thought, or English recommendation reasons inside Arabic conversation.',
    '',
