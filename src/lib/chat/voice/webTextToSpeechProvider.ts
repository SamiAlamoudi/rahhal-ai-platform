...
function pickVoice(voices: SpeechSynthesisVoice[], locale: 'ar' | 'en'): SpeechSynthesisVoice | null {
  const prefix = locale === 'ar' ? 'ar' : 'en'
  const exact = voices.find((v) => v.lang?.toLowerCase() === (locale === 'ar' ? 'ar-sa' : 'en-us'))
