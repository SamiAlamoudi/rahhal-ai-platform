...
| `ui.travel_workspace` | experimental | **no** | Phase 4 Stage 5 Premium Travel Workspace — operational journey UI only; depends on `ui.application_shell`; no booking/Amadeus/payments/AI/APIs; not wired to Chat/Voice/Knowledge (alias: travel_workspace) |
...
| `brain.conversation_orchestrator` | experimental | **no** | Phase 6 Stage 2 AI Conversation Orchestrator architecture — contracts/types only; depends on `ui.integration_foundation`; no LLM/API/Runtime/production wiring; distinct from `ai.conversation_orchestrator` (alias: brain_conversation_orchestrator) |
| `brain.planning_engine` | experimental | **no** | Phase 6 Stage 3 AI Planning Engine architecture — contracts/types only; depends on `brain.conversation_orchestrator`; no planning execution/LLM/Runtime/booking APIs/maps/weather/payments (alias: brain_planning_engine) |
| `brain.decision_engine` | experimental | **no** | Phase 6 Stage 4 AI Decision Engine architecture — contracts/types only; depends on `brain.planning_engine`; no decision execution/LLM/Runtime/booking APIs/maps/weather/payments (alias: brain_decision_engine) |
...
| `brain.runtime_orchestrator` | experimental | **no** | Phase 6 Stage 9 AI Runtime Orchestrator architecture — contracts/types only; depends on `brain.llm_adapter`; no production runtime/AI calls/APIs/SDKs/HTTP/streaming/tool execution/DB (alias: brain_runtime_orchestrator) |
| `brain.traveler_profile` | experimental | **no** | Phase 7 Stage 1 Traveler Profile Foundation architecture — contracts/types only; depends on `brain.runtime_orchestrator`; no DB/auth/storage/OCR/LLM/Runtime/HTTP/APIs; distinct from `ui.traveler_profile` (alias: brain_traveler_profile) |
| `brain.loyalty_foundation` | experimental | **no** | Phase 7 Stage 2 Loyalty Platform Foundation architecture — contracts/types only; depends on `brain.traveler_profile`; no DB/auth/payments/reward calculation/Runtime/HTTP/APIs/LLM; distinct from Sprint 38 `brain.loyalty_platform` (alias: brain_loyalty_foundation) |
| `brain.personalization_engine` | experimental | **no** | Phase 7 Stage 3 AI Personalization Engine architecture — contracts/types only; depends on `brain.loyalty_foundation`; no LLM/recommendation execution/DB/Runtime/HTTP/APIs; distinct from `ai.personalization` / `ai.recommendation_engine` (alias: brain_personalization_engine) |
| `brain.preference_extraction` | experimental | **no** | Phase 7 Stage 4 AI Smart Preference Extraction Engine architecture — contracts/types only; depends on `brain.personalization_engine`; no LLM/DB/storage/Runtime/recommendation execution/HTTP/APIs (alias: brain_preference_extraction) |
| `brain.context_engine` | experimental | **no** | Phase 7 Stage 5 Traveler Context Engine architecture — contracts/types only; depends on `brain.preference_extraction`; live conversation context (not Memory); no LLM/Runtime/DB/storage/HTTP/APIs; distinct from `brain.context_memory` (alias: brain_context_engine) |
| `brain.intent_engine` | experimental | **no** | Phase 7 Stage 6 Intent Recognition Engine architecture — contracts/types only; depends on `brain.context_engine`; no LLM/Runtime/DB/storage/HTTP/APIs; distinct from Sprint 19 `brain.intent` (alias: brain_intent_engine) |
| `brain.travel_planning` | experimental | **no** | Phase 7 Stage 7 AI Travel Planning Engine architecture — contracts/types only; depends on `brain.intent_engine`; planning structures only (never books); no Runtime/LLM/pricing/external APIs/DB/storage; distinct from Phase 6 `brain.planning_engine` (alias: brain_travel_planning) |
| `brain.search_orchestrator` | experimental | **no** | Phase 7 Stage 8 Travel Search Orchestrator architecture — contracts/types only; depends on `brain.travel_planning`; never calls providers; no Runtime/HTTP/SDKs/provider APIs/DB/storage/booking/pricing/LLM; distinct from Sprint 24 `brain.search` (alias: brain_search_orchestrator) |
| `brain.travel_recommendation` | experimental | **no** | Phase 7 Stage 9 Travel Ranking & Recommendation Engine architecture — contracts/types only; depends on `brain.search_orchestrator`; never books or contacts providers; no Runtime/LLM/HTTP/provider APIs/DB/storage/pricing; distinct from `ai.recommendation_engine` / `ai.recommendation_intelligence` / `brain.personalization_engine` (alias: brain_travel_recommendation) |
| `brain.offer_decision_engine` | experimental | **no** | Phase 7 Stage 10 Offer Decision Engine architecture — contracts/types only; depends on `brain.travel_recommendation`; never books/contacts providers/calculates payments/executes recommendations; no Runtime/LLM/HTTP/OCR/DB/storage/auth; distinct from `brain.travel_recommendation` / `brain.personalization_engine` / `ai.recommendation_engine` (alias: brain_offer_decision_engine) |
| `brain.booking_orchestrator` | experimental | **no** | Phase 7 Stage 11 Travel Booking Orchestrator architecture — contracts/types only; depends on `brain.offer_decision_engine`; prepares booking workflows only (never executes); no providers/payments/reservations/notifications/Runtime/LLM/HTTP/OCR/DB/auth; distinct from `booking.orchestrator` / `src/lib/booking` / `src/core/booking` (alias: brain_booking_orchestrator) |
| `brain.conversation_brain` | experimental | **no** | Phase 7 Stage 12 AI Conversation Brain Orchestrator architecture — contracts/types only; depends on `brain.booking_orchestrator`; coordinates Phase 7 engines through contracts only; no Runtime/LLM/HTTP/providers/booking/DB/UI; distinct from `agent/conversationBrain` / `brain.conversation_orchestrator` / `ai.conversation_orchestrator` (alias: brain_conversation_brain) |
